...
<form class="layui-form layui-form-pane" action="" hidden="hidden" id="adduser_form">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="checkbox" name="status" id="status" lay-skin="switch" lay-text="激活|未激" class="layui-btn-lg">
        </div>
    </div>
        ...
        table.on('tool(dateTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'edit'){ //修改
                ...
                layer.confirm('真的要禁用该账户么', function(index){
                    ...
                    //开启ajax请求 禁用
                    $.get("${pageContext.request.contextPath}/stopOrStartAccountServlet?option=stop&userId="+data.userId,
                    		function (reslut) {
	                        if(reslut>0)
	                        {
